...
from timesmash import _AUC_Feature, XHMMFeatures
np.random.seed()
import random
percent_run = 0.2
...
print(train_label[0].unique())

alg = XHMMFeatures(delay_max=1, delay_min=1,return_failed=True, n_quantizations = 10, self_models=True, use_flex_roc=True)
train_feature = alg.fit_transform([train], train_label)
test_feature = alg.transform([test])

test_feature.to_csv('test_feature.csv')
train_feature.to_csv('train_feature.csv')

from sklearn.ensemble import RandomForestClassifier
train_feature = train_feature.dropna(axis=1)
test_feature = test_feature.dropna(axis=1)
